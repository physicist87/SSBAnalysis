#include "TtFullLepKinSolver.hpp"
#include "TF2.h"
...
//   EventShape_->SetParameters(30.7137,56.2880,23.0744,59.1015,24.9145);
   EventShape_ = new TF2("landau2D","[0]*( ( ( TMath::Landau(x,[1],[2],0) ) * [3] + TMath::Exp( [4] / ( [5]+[6]*x*x ) ) * [7]) * ( TMath::Landau(y,[8],[9],0)*[10] + TMath::Exp( [11] / ( [12]+[13]*y*y+[14]*y) )*[15] ) )"
                                      ,0,1200,0,1200);
//   EventShape_->SetParameter(0,1);
   EventShape_->SetParameter(0,   1/4.53731e+06); //
   EventShape_->SetParameter(1,   5.81649e+01  ); // landau mpv
   EventShape_->SetParameter(2,   2.31911e+01  ); // rms
   EventShape_->SetParameter(3,   1.19038e+04  ); //
   EventShape_->SetParameter(4,  -9.78134e+01  ); //
   EventShape_->SetParameter(5,  -2.76894e+01  ); //
   EventShape_->SetParameter(6,  -2.01114e-02  );
   EventShape_->SetParameter(7,  -5.07004e+00  );
   EventShape_->SetParameter(8,   5.76608e+01  ); // landau mpv
   EventShape_->SetParameter(9,   2.34419e+01  ); // rms
   EventShape_->SetParameter(10,  1.18119e+04  ); //
   EventShape_->SetParameter(11,  5.26711e+03  );
   EventShape_->SetParameter(12,  1.64080e+03  ); //
   EventShape_->SetParameter(13,  5.86524e+00  ); //
   EventShape_->SetParameter(14, -8.07173e+01  );
   EventShape_->SetParameter(15, -5.44618e+00  );
}
...
//   EventShape_->SetParameters(nupars[0],nupars[1],nupars[2],nupars[3],nupars[4]);
   ...
   for ( int i = 0; i < nupars.size(); ++i ) {   EventShape_->SetParameter( i, nupars[i] ); }
}
...
TtFullLepKinSolver::getNuSolution(TLorentzVector LV_l, TLorentzVector LV_l_, TLorentzVector LV_b, TLorentzVector LV_b_)
{
...
//  math::XYZTLorentzVector maxLV_n_ = math::XYZTLorentzVector(0,0,0,0);   
   ...
   TLorentzVector maxLV_n;
   TLorentzVector maxLV_n_;
   //loop on top mass parameter
   double weightmax = -1;
   ...
   for(double mt = topmass_begin; mt < topmass_end + 0.5*topmass_step; mt += topmass_step) 
   {
      ...
      for (int isol = 0; isol < NSol; isol++) 
      {
         ...
         {
            weightmax =weight;
	          maxLV_n.SetPxPyPzE(LV_n.Px(), LV_n.Py(), LV_n.Pz(), LV_n.E());//LV_l
    	      maxLV_n_.SetPxPyPzE(LV_n_.Px(), LV_n_.Py(), LV_n_.Pz(), LV_n_.E());//LV_l_
         }
      }
   }// mt loop 
...
//  nuSol.neutrinoBar = reco::LeafCandidate(0, maxLV_n_ ); 
   nuSol.neutrino    = maxLV_n;
   nuSol.neutrinoBar = maxLV_n_;//  
   nuSol.weight = weightmax;
   return nuSol;
}

void TtFullLepKinSolver::FindCoeff( const TLorentzVector al,  const TLorentzVector l, const TLorentzVector b_al, 
                                    const TLorentzVector b_l, const double mt,        const double mat,
                                    const double px_miss,     const double py_miss,   double* koeficienty        )
{
...
void TtFullLepKinSolver::TopRec(const TLorentzVector al,   const TLorentzVector l,
      	                        const TLorentzVector b_al, const TLorentzVector b_l,  
                                const double sol       )
{
   TVector3 t_ttboost;
   TLorentzVector aux;
   ...
   LV_t_ = b_l + l + LV_n_;
   LV_t = b_al + al + LV_n;  
//   cout << "b_l : " << b_l.Pt() << " l : " << l.Pt() << " LV_n_ " << LV_n_.Pt()<< endl; 
//   cout << "b_al : " << b_al.Pt() << " al : " << al.Pt() << " LV_n " << LV_n.Pt()<< endl; 
   aux       = (LV_t_ + LV_t);
...
int TtFullLepKinSolver::quartic(double *koeficienty, double* koreny) const
{
   ...
   if (koeficienty[4]==0.0) { return cubic(koeficienty, koreny); }
   /* quartic problem? */
...
int TtFullLepKinSolver::cubic(const double *coeffs, double* koreny) const
{
   ...
   if (coeffs[3]!=0.0) 
   {
      ...
      if (dis<0.0) 
      {
         ...
         for(unsigned i=0; i<3; i++) {	koreny[i] = p*cos((phi+2*i*TMath::Pi())/3.0) - w; }
    ...
    /* Perform one step of a Newton iteration in order to minimize
      ...
      for(unsigned i=0; i<nreal; i++) 
      {
         ...
         if (h != 0.0) {koreny[i] -= (coeffs[0] + koreny[i] * (coeffs[1] + koreny[i] * (coeffs[2] + koreny[i] * coeffs[3])))/h;}
      }
   }
...
void TtFullLepKinSolver::SKTEST()
{
   cout << "TtFullLepKinSolver test!!! " << endl;
}
void TtFullLepKinSolver::SKTEST1( const TLorentzVector al )
{
   cout << "SKTEST1 " << al.Pt() << endl;
}
